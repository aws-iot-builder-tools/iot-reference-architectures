...
public class DeleteAllCommandHandler implements CommandHandler {
    private final Logger log = LoggerFactory.getLogger(DeleteAllCommandHandler.class);
    private static final String DELETE = "delete-all";
    @Inject
    ...
    public void innerHandle(String input) {
        ...
        while (messageId != null) {
            if (!sharedMqtt.deleteMessage(messageId).isPresent()) {
                log.error("No delete response received");
                return;
            }

            log.info("Deleted " + messageId);
    ...
    public String getHelp() {
        return "Deletes all available messages.";
    }
